					...
					try {
						...
						try {
							...
							if (log.isInfoEnabled()) {
								...
								builder.append("\r\n │ \t ├ 已接收消息(packet/byte):").append(serverGroupStat.getReceivedPackets().get()).append("/")
										.append(serverGroupStat.getReceivedBytes().get());
								builder.append("\r\n │ \t ├ 已发送消息(packet/byte):").append(serverGroupStat.getSentPackets().get()).append("/")
										.append(serverGroupStat.getSentBytes().get()).append("b");
